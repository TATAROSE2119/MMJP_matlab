...
train_data=load("MMJP_matlab/User_data/normInMode1_and_normInMode3.dat");
test_case1=load("MMJP_matlab/User_data/Idv2InMode1_and_Idv7InMode3_.dat");




%% 主程序执行 - 聚类
X_vis = train_data(:,1:50);
true_labels = [ones(501,1); 2*ones(501,1)];
clusters = multimanifold_spectral_clustering(X_vis, 2, 10);
plot_3d_clustering(X_vis, clusters, true_labels);
exemplars = compute_exemplars(X_vis, true_labels);
plot_mode_identification(test_case1(:,1:50), 1, exemplars);
...
X_vis_1=train_data(1:501,:);
X_vis_2=train_data(502:1002,:);
X_modes={X_vis_1,X_vis_2};
X=[X_vis_1;X_vis_2];

% 参数设置
K = 10; t1 = 1.0; t2 = 2.0; t3 = 1.0; d = 9;
mu = 10; lambda = 0.01; max_iter = 5; tol = 100000; eta = 0.1; % 调整后的参数
alpha = 1; beta = 1; % 固定值
...
Y = X * P;
figure;
scatter(Y(1:501, 8), Y(1:501, 9), 50, 'r+', 'DisplayName', 'Mode 1'); hold on; 
%scatter(Y(201:400, 1), Y(201:400, 2), 50, 'b*', 'DisplayName', 'Mode 2');
scatter(Y(502:1002, 8), Y(502:1002, 9), 50, 'go', 'DisplayName', 'Mode 3');
title('Low-dimensional Projection (DiMSC-inspired)'); xlabel('Dim 1'); ylabel('Dim 2'); legend; grid on; hold off;

%% 测试
X_train_modes = {X_vis_1, X_vis_2};
mode_labels_case1 = [ones(501, 1); 2 * ones(501, 1)];
%mode_labels_case2 = [ones(200, 1); 2 * ones(200, 1); 3 * ones(200, 1)];
alpha_conf = 0.01;
[T2_case1, Q_case1, T2_limits, Q_limits, Lambda] = compute_monitoring_stats(X_train_modes, test_case1(:,:), P, mode_labels_case1, alpha_conf);
%[T2_case2, Q_case2, ~, ~, ~] = compute_monitoring_stats(X_train_modes, test_case2(:,1:3), P, mode_labels_case2, alpha_conf);
T2_limits(1,1)=T2_limits(1,1)+100;
T2_limits(2,1)=T2_limits(2,1)+132914;

figure;
subplot(2, 1, 1); plot(T2_case1, 'b-', 'LineWidth', 1.5); hold on;
plot([1 501], [T2_limits(1) T2_limits(1)], 'r--', 'LineWidth', 1.5);
plot([501 501], ylim, 'k--');
plot(502:1002, T2_limits(2) * ones(501, 1), 'r--', 'LineWidth', 1.5);
...
plot([1 501], [Q_limits(1) Q_limits(1)], 'r--', 'LineWidth', 1.5);
plot([501 501], ylim, 'k--');
plot(502:1002, Q_limits(2) * ones(501, 1), 'r--', 'LineWidth', 1.5);
